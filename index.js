...
// Owner numbers
const ownerNumber = ["923229309266"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/arslan.html')) {
    ...
    console.log("⚠️ ABDULLAH.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  console.log("♻️ ABDULLAH-MD session restored successfully");
}
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://i.ibb.co/bgNjJjNZ/20260520-102504.jpg'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 ABDULLAH-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ ABDULLAH-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *ABDULLAH-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ ABDULLAH MD ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using ABDULLAH MD`;
      
      // Image URL for connection message
      const imageUrl = 'https://i.ibb.co/bgNjJjNZ/20260520-102504.jpg';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923229309266@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://i.ibb.co/bgNjJjNZ/20260520-102504.jpg');
